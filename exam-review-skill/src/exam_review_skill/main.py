...
# ---- v2 imports (LLM + Graph) ----
try:
    from .llm import chunk_from_parsed_doc, analyze_chunks, run_rule_fallback
    from .graph import build_graph, build_graph_rule_fallback, export_d3_html
    _V2_AVAILABLE = True
except ImportError:
    _V2_AVAILABLE = False
...
def run(input_data: dict[str, Any]) -> dict[str, Any]:
    ...
    # ---- 3. 知识结构化（v2: LLM 优先 / v1: 规则引擎 fallback）----
    analysis_mode = input_data.get("config", {}).get("analysis_mode", "rule")

    llm_result = None
    if analysis_mode == "llm" and _V2_AVAILABLE:
        try:
            all_chunks = []
            ...
            if all_chunks:
                from .llm.chunker import ChunkedDocument
                combined = ChunkedDocument()
                combined.chunks = all_chunks
                combined.total_chunks = len(all_chunks)
                llm_result = analyze_chunks(combined)
                if llm_result.get("analysis_mode") == "unavailable":
                    llm_result = None  # 降级
        except Exception:
            llm_result = None

    # 规则引擎始终运行（作为 fallback 或对比基准）
    ...
    if _V2_AVAILABLE:
        try:
            if llm_result and llm_result.get("knowledge_points"):
                kg = build_graph(llm_result, course_name=course.course_name)
            else:
                kg = build_graph_rule_fallback(tree, course_name=course.course_name)
            kg_html = export_d3_html(kg)
        except Exception:
            pass
    ...
    focus = config.focus_preference  # "概念"/"公式"/"例题"/"定理证明"/"均衡"

    if "basic_concepts" in modules:
        concepts = generate_basic_concepts(tree)
        if focus != "均衡":
            concepts = _filter_by_focus(concepts, focus)
    ...
    if "high_freq_points" in modules:
        ...
        if focus != "均衡":
            hf_points = _filter_by_focus(hf_points, focus)
    ...
    if "error_prone" in modules:
        ...
        if focus != "均衡":
            errors = _filter_by_focus(errors, focus)
    ...
    # v2 增强
    output.data["analysis_mode"] = "llm" if llm_result else "rule"
    if llm_result:
        output.data["llm_knowledge_points"] = llm_result.get("knowledge_points", [])
        output.data["llm_relationships"] = llm_result.get("relationships", [])
    if kg_html:
        output.data["knowledge_graph_html"] = kg_html
...
def _filter_by_focus(items: list[dict], focus: str) -> list[dict]:
    """按 focus_preference 过滤内容项"""
    focus_keywords = {
        "概念": ["定义", "概念", "是", "指", "称为", "含义"],
        "公式": ["公式", "=", "计算", "+", "-", "*", "/", "函数", "方程"],
        "例题": ["例", "例题", "解：", "解答", "示例"],
        "定理证明": ["定理", "证明", "推导", "公理", "引理", "定律"],
    }
    ...
    filtered = []
    for item in items:
        text = item.get("name", "") + " " + item.get("content", "") + " " + item.get("description", "")
        if any(kw in text for kw in keywords):
            filtered.append(item)
    # 如果过滤后为空，返回原始列表（避免空结果）
