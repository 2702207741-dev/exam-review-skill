...
@click.version_option(version="1.0.0", prog_name="exam-review")
def cli(input_file: str | None, output: str | None,
        demo: bool, pretty: bool) -> None:
    """
    Exam Review Skill — 期末复习知识整理
    ...
    if not input_file:
        click.echo("请提供输入 JSON 文件，或使用 --demo 运行示例", err=True)
        sys.exit(1)
    ...
    if code == 0:
        click.echo(f"\n✅ 处理成功 - {result['data'].get('course_summary', {}).get('knowledge_point_count', 0)} 个知识点")
    else:
        click.echo(f"\n⚠️ 处理完成 (code={code}): {result.get('message', '')}")
